# ADCREO — Blueprint Execution File (BEF) v2

> **Classification:** CONFIDENTIAL · GOLDEN RC · Execution-Grade Blueprint
> **Status:** MVP Locked · Aligned to PRD v4 (queued) + TDD v5 (19 May 2026)
> **Version:** v2 — supersedes BEF v1 in every section touched by the v4-Δ1 B-roll layer and corrects pre-existing inconsistencies surfaced during architect review. Sections not listed in the V2 CHANGELOG are unchanged from v1.
> **Audience:** Google Antigravity Agentic IDE (executor) + Founder (orchestrator)
> **Sprint Window (historical):** Apr 26 → May 10 (15-day build), May 10 → May 20 (10-day closed beta), May 20 (production launch). The sprint calendar is preserved as historical record; current build state is at micro-phase 5 (Cross-Cutting Hardening), G1/G2 cleared, G3/G4 partial — see §16.13 below.
> **Invariants:** 6-Screen Linear Flow · 4-Phase Pipeline · Mandatory Intent Gate · Bounded Co-Pilot Chat · Credit Lock Before Compute · State-Driven UI · 2xx-Only Idempotency Cache · Origin-Preserving Webhook Restore · Role-First Deterministic B-Roll Selection *(v4-Δ1)*
...
The Blueprint Execution File (BEF) is the third-tier authority document in the ADCREO specification stack. PRD defines **WHAT** must exist; TDD defines **HOW** it is implemented; this BEF defines **HOW TO EXECUTE THE BUILDING OF IT** — the order, the seams, the human-vs-agent boundary, and the validation gates between PRD requirements and TDD code.
...
- It does not invent features, screens, FSM states, ENUM values, error codes, or endpoints. Anything not present in PRD or TDD is out of scope.
...
To make traceability mechanical. Bi-directional traceability is enforced by `[TDD-TRACE]` in TDD v5 and is reproduced here as the BEF execution map. Every PRD anchor that the Agentic IDE will encounter at execution time has a TDD implementation anchor and a BEF section that sequences it.
...
- The list of conflicts already resolved against the current document revisions.
...
| `[PRD-MOATS]` (Style Memory) | `[TDD-FLYWHEEL]-A/B`, `[TDD-SCHEMA]` `user_style_profiles` (incl. `brand_tier`, `brand_color_primary` v4-Δ1) | §11 |
...
| **F-405 B-Roll Planner** *(v4-Δ1)* | `[TDD-BROLL]` (full chapter, authoritative) + `[TDD-SCHEMA]` (`broll_clips`, `broll_clip_usage`, `seasons`) + `[TDD-WORKERS]-G` + `[TDD-VIDEO]-A/B` + `[TDD-WORKERS]-C2` *(supersede pointer)* | §9.6, §11, §13.9 |
...
| C5 | C2PA verification target | `[PRD-FEATURES-COMPLIANCE]`: External verification (Adobe VerifyContentCredentials) is **forbidden** in CI and worker loops. | `[TDD-WORKERS]-I`: c2patool returncode-checked; local `c2patool --verify` for verification. | **PRD wins (Axis A1) and TDD agrees.** All C2PA verification is local via `c2patool --verify`. External tools are end-user optional only. See GAP-07 in §16.7 for the historical patch. The pre-v2 wording of Gate 4 in §15.7 referencing Adobe is corrected in this revision. |
| C6 | Migration runner | PRD silent. | `[TDD-IDE]-A` step 2a: raw SQL via `ci/run_migrations.py --dry-run` / `--apply`, lexicographic order, recorded in `schema_migrations`. | **TDD wins (Axis A2).** No Alembic. See GAP-09 in §16.7 for the canonical resolution. **(v2 cleanup):** the §15.10 rollback drill text in BEF v1 referenced `alembic downgrade -1` and `Helm rollback` — both wrong. Corrected in this revision. |
| C7 | Alerting platform | PRD silent on alerting backend. | `[TDD-OBSERVABILITY]`: PostHog + Grafana Cloud; PagerDuty not used in MVP. | **TDD wins (Axis A2).** Alert tiers (`GA-Warn`, `GA-Critical`) are abstract severities; delivery is via Grafana per `[PRD-FEATURES-INFRA]` F-606. References to "PagerDuty P0" in earlier drafts are normalized to "Grafana alert" in §14 and §15. v1 leftover "PD-Warn / PD-Critical" labels in §15.11 are normalized to `GA-Warn` / `GA-Critical` in v2. |
| **C8** *(NEW v2)* | B-Roll taxonomy | `[PRD-NON-NEGOTIABLES]` and PRD F-405: "abstract, motion, texture, warehouse, packaging" (curatorial categories); the term "warehouse" implies an identifiable location. | `[TDD-BROLL]`: role-first taxonomy (`hook` / `context` / `cta_bg`), with mood + seasonality + brand-tier metadata; clips remain location-agnostic per `chk_broll_safety`. | **TDD wins (Axis A2).** The PRD taxonomy is reframed in PRD v4 as **curatorial categories at upload time only**; runtime selection is role-first per `[TDD-BROLL]-D/E/G`. The string "warehouse" is deprecated in PRD v4 because it contradicts `[PRD-NON-NEGOTIABLES]` U1 (Product-Visual First — no recognizable locations). PRD v4 ticket open; until merged, BEF v2 references reflect the TDD V5 taxonomy. |
| **C9** *(NEW v2)* | Tier-2 degradation semantics | TDD v3 / BEF v1: "B-roll intercut disabled" (Worker-COMPOSE emits I2V-only). | TDD V5 `[TDD-MIGRATIONS]` (14): "force T1/text-card-CTA fallback (planner skips role lookups; Worker-COMPOSE uses neutral text-card)". | **TDD wins.** Tier-2 no longer disables B-roll wholesale — it forces the planner to short-circuit to T1 with hook+CTA-text-card while the library has a coverage gap. Reflected throughout §13.9 in v2. |
...
| Toggle feature flags (Tier 1–4 degradation, beta gate, `broll_v4_routing_enabled`) | ✅ exclusive | ❌ | Production behavior change. |
| Acknowledge a `GA-Critical` Grafana alert | ✅ exclusive | ❌ | Human accountability. |
...
| Enable a new season row in `seasons` *(v4-Δ1)* | ✅ exclusive | ❌ | `is_active` flip is a production-behavior change; ops/marketing call. |
| Edit `app/broll/templates.py` (TEMPLATE_BY_CONTEXT / MOOD_BY_ANGLE) *(v4-Δ1)* | ✅ exclusive | ❌ | Frozen routing dict; CI-locked. |
...
| **G3** *(v2 — rewritten for v4-Δ1)* | Step 20 — B-Roll Planner (`app/broll/planner.py`) + 5-template router (`app/broll/templates.py`) | Coverage + bounded determinism per `[TDD-BROLL]-G/J/K`. The byte-identical determinism test from BEF v1 is **deprecated** — the new contract includes rotation history and freshness-aware selection, so two runs by the same user can legitimately differ. | (i) `ci/broll_coverage_check.py` green: every `(angle, category_class, brand_tier)` triple resolvable via `TEMPLATE_BY_CONTEXT` has ≥ 1 active clip for each required role of the resolved template, and every active `seasons.season_tag` has ≥ 1 hook + 1 context clip carrying that tag in `seasonality_tags`. (ii) **Bounded determinism:** for a fresh user (empty `broll_clip_usage`), two consecutive `BRollPlanner.plan(template, angle, category, brand_tier, user_id, active_season)` calls with identical inputs return byte-identical output. (iii) **Rotation-aware test:** after one plan persists usage rows, the next call with the same inputs deprioritizes the just-used clip per `[TDD-BROLL]-G` step 4 (different clip selected if alternatives exist; same clip permitted only when no alternative exists, per the coverage check). (iv) **Plan shape:** verify the persisted `b_roll_plan` is a JSONB object `{template, hook, context, cta}` and passes the rewritten `chk_b_roll_plan_shape` CHECK. (v) `d2c_beauty + emotion` resolves to T1 (Bookended) and returns sensory-archetype hook. |
| **G4** *(v2 — corrected per C5)* | Step 26 — Worker-EXPORT functional end-to-end | Full HD-1→HD-6 flow: 15s render completes, declarations capture, `c2patool --version` runs, both 1080×1080 + 1080×1920 sign with returncode 0. | C2PA manifest validates using **local `c2patool --verify`**. External verification APIs (Adobe VerifyContentCredentials) are **strictly forbidden** in both CI and runtime per `[PRD-FEATURES-COMPLIANCE]`. SGI watermark visible at both resolutions. Worker-COMPOSE branches per `structure_template`; if `b_roll_plan.cta` is null (T2/T3/T4/T5), verify the FFmpeg-rendered text-card CTA appears at the correct timestamp per `[TDD-BROLL]-C`. |
...
- Writing `httpx`, `requests`, or `aiohttp` imports inside `app/workers/strategist.py`, `compose.py`, `export.py`, `copy.py`, `critic.py`, `tts.py`, `i2v.py`, `reflect.py`, **or anywhere under `app/broll/`** *(v4-Δ1 sandbox extension per `[TDD-IMPORT-GRAPH]`)*.
- Writing `sentence_transformers`, `chromadb`, `faiss`, `faiss-cpu`, `transformers`, or `torch` imports anywhere under `app/broll/` *(v4-Δ1 — defense against drift toward embedding-based clip selection; `[TDD-BROLL]-J`)*.
...
- Using presigned R2 URLs inside any worker. **Storage Security Invariant:** B-roll and source assets are stored in Cloudflare R2 with curatorial folder organization (see §11.8 — folder names are organizational only, not semantic; clip selection is metadata-driven via `broll_clips` columns per `[TDD-BROLL]-E`). Workers must use credentialed `boto3` access only; presigned URLs are strictly forbidden for internal worker-to-worker transfers.
...
- **Editing `app/broll/templates.py` (the frozen TEMPLATE_BY_CONTEXT routing dict)** — Founder-exclusive per §3.4; the agent may only **read** this module.
- **Flipping `seasons.is_active = TRUE` for any row** — operator-only action per §11.4.
- **Inserting rows into `broll_clips` outside the curation pipeline** — curation is a Founder/curator activity; the agent only consumes the table.
...
This section states in one bounded definition exactly what ADCREO MVP is — and by negation, what it is not — so that no agent task may invent a feature outside this envelope.
...
6. An **opt-in Style Memory upsert** in `user_style_profiles` (pgvector embedding of the winning script + framework + motion + environment + brand_tier; `brand_color_primary` optionally captured via onboarding), reusable for the user's next generation in the same category. *(v4-Δ1: `brand_tier` drives B-roll template routing; `brand_color_primary` backs the text-card CTA gradient when not NULL — neutral charcoal fallback otherwise.)*
...
| Product categories | 5 Green Zone: `d2c_beauty`, `packaged_food`, `hard_accessories`, `electronics`, `home_kitchen` | All others (rejected at Phase 1 with `failed_category`) |
...
|---|---|---|
| Render duration | Canonical 15 seconds; segment structure per `structure_template` (T1..T5 per `[TDD-BROLL]-C`). T1 (default Bookended): 3s hook B-roll + 9s I2V + 3s CTA B-roll. T2..T5 substitute an FFmpeg-rendered text-card CTA for the CTA clip. | Any other total duration in MVP |
| Hero asset | The user's actual product (isolated PNG via BiRefNet (Apache 2.0)) plus optional curated atmospheric B-roll (role-tagged hook / context / cta_bg; faces / hands / identifiable locations excluded by `chk_broll_safety`) | Generic stock product footage · talking-head avatars · human spokespersons · lip-sync · video-to-video stylization |
...
Per `[PRD-COMPETITION]` and `[PRD-DIFFS]`, ADCREO occupies the **uncontested upper-right quadrant**: HIGH India localization × HIGH I2V × IT Rules 2026 compliance baked-in. The three strategic moats are:

1. **Regulatory defensibility:** SGI burn-in + C2PA signing + 5-year partitioned audit + <60-min auto-takedown.
2. **Hyper-localization:** Sarvam Bulbul V3 routing for 7 Indic TTS languages; festive template override (T4) bound to `seasons` table for Diwali / Holi / wedding / monsoon / new-year / raksha-bandhan windows *(v4-Δ1)*.
...
**Plan-tier × B-roll template interaction** *(v4-Δ1, resolved per TDD V5 CONSISTENCY-FLAG F6):* `plan_tier` and `structure_template` are **decoupled** in v1. Template selection depends on `(framework_angle, category_class, brand_tier, active_season)` per `[TDD-BROLL]-G`, **never** on `plan_tier`. Rationale: `brand_tier` is per-brand (a Starter user with a premium-watch brand should still get T2); `plan_tier` is per-account and already gates rendering capability. Conflating the two would give a Starter user with a luxury brand a mass-market template, which is wrong for both the user and the brand. The `broll_v4_routing_enabled` feature flag (default FALSE) is the global kill-switch if T2–T5 prove too expensive at scale; re-evaluate post-launch with real cost data. Until the flag is flipped, every render uses T1 — exact v4 baseline behavior.
...
| **F9** *(NEW v2)* | **Zero Marginal Cost B-Roll Selection.** The B-roll planner makes zero external API calls, runs no LLM, performs no vector search; selection is a single deterministic SQL read with total-ordered tiebreaks. | `[TDD-BROLL]-G/J` + `ci/strategist_sandbox_check.py` extended to `app/broll/` per `[TDD-IMPORT-GRAPH]`. |
...
| **L8** *(NEW v2)* | **B-Roll Safety Purity.** Every clip in `broll_clips` has `excludes_faces = excludes_hands = excludes_locations = TRUE` (DB CHECK `chk_broll_safety`). Additionally, every `role='cta_bg'` clip has `negative_space_score >= 7` for text-card legibility (DB CHECK `chk_cta_negative_space`). | DB constraints + curation discipline. |

### 5.6 UX & Product Identity Invariants

| ID | Invariant | Enforcement |
|---|---|---|
| **U1** | **Product-Visual First.** The user's actual product is the hero asset in every ad. Stock product footage and avatar-led ads are forbidden. | Product comes from `isolated_png_url` (BiRefNet (Apache 2.0)). B-Roll planner excludes faces/hands/locations via `chk_broll_safety` constraint. The "warehouse" curatorial category from PRD v3 — which implied a recognizable location — is deprecated in PRD v4 per Conflict C8. |
| **U2** | **Green Zone Restriction.** Only 5 categories accepted. All others → `failed_category`. | `[PRD-GREENZONE]` ENUM + Worker-EXTRACT raises `CategoryError`. |
...
| **U6** *(NEW v2)* | **Role-First B-Roll.** A clip's role (`hook` / `context` / `cta_bg`) is fixed at curation time and never overridden by selection logic. The planner filters by role first, then by category × brand_tier × seasonality; ORDER BY produces a total ordering. | `[TDD-BROLL]-D/G/K` invariant 2 (role purity). |
...
| **S13** *(NEW v2)* | **B-Roll Plan Atomic Persistence.** Worker-STRATEGIST's UPDATE of `generations.b_roll_plan + structure_template + active_season` AND the INSERTs into `broll_clip_usage` for every non-null clip in the plan happen inside a single `db.transaction()`. A partial write would corrupt rotation determinism on the next generation for this user. | `[TDD-WORKERS]-G` v4-Δ1 step 5. |
| **S14** *(NEW v2)* | **B-Roll Plan Shape.** `generations.b_roll_plan` is JSONB-object-shaped `{template, hook, context, cta}` with `template ∈ {T1..T5}`, `hook` non-null, `context` and `cta` independently nullable. NULL plan is permitted (pre-Phase-3 or strategist deferred). | `chk_b_roll_plan_shape` rewritten in TDD V5 from array-form to object-form. |
...
| **Z9** *(NEW v2)* | **B-Roll Templates Freeze.** `app/broll/templates.py` (`TEMPLATE_BY_CONTEXT`, `MOOD_BY_ANGLE`, `CATEGORY_CLASS`, `DEFAULT_TEMPLATE='T1'`, `FESTIVE_OVERRIDE_TEMPLATE='T4'`) is CI-locked. The dict's keys and values may not be changed without a TDD V6 revision. | CI snapshot test in `tests/contract/test_broll_templates_frozen.py`. |

### 5.9 Dependencies with other sections

§5 is referenced by every later section. §7 (FSM) operationalizes S6, S7. §8 (Phase Architecture) operationalizes S10, S11, S13. §12 (API Surface) operationalizes S3, S4, S5, S8. §13 (Failure Logic) operationalizes F2, F3, S11. §14 (Compliance) operationalizes L1..L8. §15 (Testing) verifies all of them.
...
- **IF** category resolves outside Green Zone → **THEN** transition to `failed_category`, emit ECM-001, surface `[Start Over]` (fresh `gen_id`); **ELSE** continue.
...
- **IF** the scraped image exceeds 15MB → **THEN** raise `CategoryError` and emit ECM-001 with `[Start Over]`; **ELSE** continue to BiRefNet.
...
|---|---|---|---|
| Product | `brief_ready` | scripts, scores, strategy, selection, motion, scene, `b_roll_plan`, `structure_template`, `active_season`, `b_roll_performance_signal` | Phase 2 full |
| Targeting | `scripts_ready` (then `regenerating`) | strategy_card, scripts, critic_scores, safe_scripts, `b_roll_plan`, `structure_template`, `active_season`, `b_roll_performance_signal` | Phase 2 full |
| Script | `scripts_ready` | strategy_card, `b_roll_plan`, `structure_template`, `active_season`, `b_roll_performance_signal` | STRATEGIST only (re-runs on next `/advance`) |
| Style | `scripts_ready` | strategy_card, `b_roll_plan`, `structure_template`, `active_season`, `b_roll_performance_signal`, motion, scene | STRATEGIST only (re-runs on next `/advance`) |

> **v2 note:** Chip-change NULL reset is extended per TDD V5 invariant S13: the strategist's three v4-Δ1 outputs must be nulled alongside the legacy fields, otherwise a re-run would write into a row that still carries the previous plan and corrupt the rotation history. `broll_clip_usage` rows are NOT deleted — they are historical per-(user, clip, gen) and remain valid; CASCADE will sweep them only if the entire generation row is deleted.
...
**User sees:** Progress bar with percent, ETA seconds remaining, four named stage rows (Voice · Render · Select · Compose) with per-stage icons (⏳ pending, 🔄 running, ✅ complete). The "B-roll" stage row is shown only when `strategy_card.b_roll_available` is TRUE (i.e., `b_roll_plan` is non-null AND `b_roll_plan.hook` is non-null) — every template guarantees a hook per `[TDD-BROLL]-K` invariant 5, so in practice this is always TRUE except during Tier-2 degradation. Provider-fallback toast appears as a transient banner if the gateway swaps a provider. On DLQ failure, an inline error card with "Your credit has been refunded" plus `[↻ Try Again]` and `[← Back to Strategy]`.

**User does:** Watches passively. On failure, clicks Try Again or Back to Strategy.

**System does:** `phase4_coordinator` runs the smart-stitch pipeline: parallel TTS + LLM prompt-preflight, then I2V attempt 1 (sequential early-exit), Worker-REFLECT (SSIM + deformation guard), fallback I2V attempt 2 if needed, then Worker-COMPOSE. Worker-COMPOSE branches per `structure_template` (T1..T5 per `[TDD-BROLL]-C`): T1 stitches 3s hook + 9s I2V + 3s CTA-clip; T2 stitches 2s hook + 11s I2V + 2s text-card CTA; T3 stitches 3s hook + 4s context + 5s I2V + 3s text-card CTA; T4 stitches 3s festive-hook + 4s festive-context + 5s I2V + 3s text-card CTA; T5 stitches 2s hook + 10s I2V + 3s text-card-with-urgency CTA. All five sum to canonical 15.000s. LUT colour grade by benefit, SGI drawtext at bottom-left, AAC audio padded/trimmed to 15s via `tpad`/`apad` — never `-shortest` per Z8. On success: `composing → preview_ready` → HD-6.
...
> **v2 note:** The HD-5 rows in this matrix ("Composing final video…" etc.) are unchanged in copy but now describe a template-driven segment graph rather than the v3 fixed 3-stream concat — the user-facing UI rows are template-agnostic. The component reads `strategy_card.b_roll_available` (per §6.9) to conditionally show the B-roll stage row; the copy for each stage row does not change.
...
- Phase 3 (Intent Gate): the mandatory HD-4 halt + the 4 [Edit] back-paths + the wallet lock entry point + v4-Δ1 template/season resolution.
- Phase 4 (Production): the smart-stitch render (template-branched) + the decoupled Worker-EXPORT.
...
- **IF** category resolves outside Green Zone → **THEN** raise `CategoryError` → ECM-001.
...
4. Worker-SAFETY: per-script moderation + PII regex (phone, email, Aadhaar) + competitor-name denylist. **IF** all 3 are rejected → **THEN** raise `SafetyError`; the caller auto-retries once with SAFE_TRIO; **ELSE** continue. Per GAP-11, partial passes (1 or 2 safe scripts) render only the safe tiles on HD-3 — never a placeholder; SSE payload gains `scripts_available` and `rejected_frameworks` fields.
...
**Chip-change re-runs:** A chip change on HD-3 triggers `POST /regenerate` which UPDATEs the chip and NULLs all downstream fields — scripts, scores, refined_script, chat_history, chat_turns_used, motion, environment, **`b_roll_plan`, `strategy_card`, `structure_template`, `active_season`, `b_roll_performance_signal`** *(v4-Δ1 — the strategist's v4-Δ1 outputs must be reset alongside the legacy fields)* — enqueues `phase2_chain` again, and resets chat counter to 3. State `scripts_ready → regenerating → scripts_ready`. `broll_clip_usage` rows are NOT deleted; rotation history is per-(user, clip, gen) and survives a regenerate.
...
**Worker involved:** Worker-STRATEGIST. **ZERO external API access** — CI-enforced via `ci/strategist_sandbox_check.py` (AST-based import guard blocks `httpx`, `requests`, `aiohttp`, `openai`, `anthropic`, `google.genai`, `boto3` from the strategist file; v4-Δ1 extends the same rules to all of `app/broll/` and additionally blocks `sentence_transformers`, `chromadb`, `faiss`, `transformers`, `torch`).

**Reads:** `safe_scripts`, `user_style_profiles` (Style Memory — including `brand_tier` and `brand_color_primary` v4-Δ1), `health:{provider}` (Redis DB3), `cogs:{gen_id}` (Redis DB2), `director_tips`, `chat_turns_used`, `broll_clips`, **`seasons`** *(v4-Δ1)*, **`broll_clip_usage`** *(v4-Δ1, for rotation deprioritization)*.

**Five-step v4-Δ1 flow** *(supersedes the v3 single-call `BRollPlanner.plan(framework_angle, category)` contract per `[TDD-WORKERS]-G`):*

1. **Resolve `active_season`** — single-row read against `seasons` WHERE `is_active = TRUE` AND `NOW()::DATE BETWEEN active_from AND active_to`. Returns `season_tag` or NULL.
2. **Resolve `brand_tier`** — read from `user_style_profiles (user_id, category)`. Cold-start defaults to `'mass'`. `brand_color_primary` is read alongside; surfaced to Worker-COMPOSE via the strategy_card.
3. **Resolve `structure_template`** — precedence order: (a) feature-flag gate (`broll_v4_routing_enabled = FALSE` → T1 short-circuit, exact v4 baseline behavior); (b) festive override (`active_season != NULL` → T4); (c) `TEMPLATE_BY_CONTEXT[(framework_angle, category_class, brand_tier)]` lookup; (d) `DEFAULT_TEMPLATE = T1`.
4. **Call `BRollPlanner.plan(template, angle, category, brand_tier, user_id, active_season)`** — deterministic SQL, no LLM. Returns `{template, hook, context, cta}` JSONB object. Hook is guaranteed non-null by the CI coverage check; `context` and `cta` are independently nullable per the template's needs.
5. **Persist atomically inside one `db.transaction()`** — UPDATE `generations.b_roll_plan + structure_template + active_season` AND INSERT one row into `broll_clip_usage` for every non-null clip in the plan (hook always; context/cta conditionally). Per S13, a partial write would corrupt rotation determinism.

**Writes:** `strategy_card JSONB` (now includes `structure_template`, `active_season`, `brand_color_primary`, `b_roll_available`), `b_roll_plan JSONB` (object-shaped per S14), `structure_template VARCHAR(5)`, `active_season VARCHAR(30)`, plus 1–3 INSERTs into `broll_clip_usage`.
...
**The four [Edit] back-paths (per §6.8):** Each NULLs the appropriate downstream fields (including the v4-Δ1 fields per the v2 chip-change rule above) and conditionally re-enqueues `phase2_chain` (for product/targeting) or just sets the status back (for script/style).

**The credit-lock entry point — `POST /approve-strategy`:** unchanged from v1 — see §12.5 canonical 5-step API anatomy and the L2 implementation in `[TDD-API]-D`.
...
3. **Sequential early-exit:** I2V attempt 1 (variable-length clip per `structure_template`; default 9s for T1; via Fal.ai primary, Minimax fallback) → state `rendering → reflecting` → Worker-REFLECT (SSIM + deformation guard).
...
6. **Worker-COMPOSE branches per `structure_template`** *(v4-Δ1 per `[TDD-VIDEO]-B` / `[TDD-BROLL]-H`):* downloads the hook clip always; downloads context and/or CTA clips per template; for T2/T3/T4/T5 the CTA segment is FFmpeg-rendered as a text-card via `lavfi color=0x<brand_color>:s=1080x1920:d=<dur> + drawtext` — the same drawtext pattern as the F-403 SGI watermark, no new dependencies. The `filter_complex` graph is variable-arity (`concat=n=len(segments):v=1:a=0`) so all five templates flow through one code path. LUT colour grade by benefit, SGI drawtext at bottom-left, AAC audio padded/trimmed to canonical 15s via `tpad`/`apad` — never `-shortest` per Z8. When `brand_color_primary IS NULL`, Worker-COMPOSE falls back to neutral charcoal `#1A1A1A`.
...
§8 implements §7's transitions. §9 details the LLM orchestration inside Phase 2 and inside the coordinator's prompt-preflight call (§9.6 covers the deterministic B-Roll Planner). §11 details the worker-permission matrix and the schema rows each worker reads/writes. §12 details the API entry points that trigger each phase. §13 details what happens when each phase fails. §14 details the compliance writes (audit_log, c2pa) embedded in Phase 4.

---

## 9. Wire Playbook Routing and Co-Pilot Chain

This section defines the bounded AI orchestration patterns that distinguish Adcreo from open-ended generative tools: the 12-framework dynamic playbook, the 5-stage Co-Pilot Chat chain, the deterministic role-first B-Roll Planner *(v4-Δ1)*, and the ModelGateway with capability-keyed routing and circuit-breaker FSM.
...
- The deterministic role-first B-Roll Planner with 5-template router *(v4-Δ1)*.
...
### 9.6 Deterministic Role-First B-Roll Planner *(v4-Δ1 — rewritten)*

**Authoritative spec:** `[TDD-BROLL]` (full chapter — A through K) is the source of truth for selection logic, template definitions, role semantics, schema, invariants, and CI guards. The supersede pointer at `[TDD-WORKERS]-C2` keeps PRD F-405 traceability resolvable.

**Why deterministic:** the planner avoids LLM calls, embeddings, and vector search to keep Worker-STRATEGIST true to its zero-external-API invariant. The contract is bounded-deterministic — given identical `(template, angle, category, brand_tier, user_id, active_season)` AND identical rotation history (`broll_clip_usage`), the planner returns byte-identical output. Rotation history is part of the input, not a side-effect.
...
BRollPlanner.plan(
    ...
    angle: str,
    category: green_zone_category,
    brand_tier: Literal['mass','premium','luxury'],
    user_id: UUID,
    active_season: str | None,
) -> BRollPlan
```

Returns a JSONB-object-shaped `BRollPlan = {template, hook, context, cta}` where `hook` is always a non-null `BRollClip`, `context` is non-null only for T3/T4, `cta` is non-null only for T1 (a clip) — T2/T3/T4/T5 emit `cta=None` and Worker-COMPOSE renders a text-card backdrop instead.

**Filtering pipeline (SQL against `broll_clips`):**

1. `role = $required_role` (the planner runs one query per role required by the resolved template — hook always; context for T3/T4; cta_bg for T1 only).
2. `category = $category::green_zone_category`.
3. `$brand_tier = ANY(brand_tier_fit)` — the clip explicitly opts into the user's tier.
4. `is_active = TRUE`.
5. `excludes_faces = TRUE AND excludes_hands = TRUE AND excludes_locations = TRUE` (also enforced by `chk_broll_safety` at insert time — belt-and-braces).
6. For T4 (festive override): `seasonality_tags && ARRAY[$active_season]::TEXT[]` (GIN-indexed array overlap).
7. For `role='cta_bg'` clips: `negative_space_score >= 7` (also enforced by `chk_cta_negative_space`).
8. **Rotation deprioritization:** LEFT JOIN `broll_clip_usage` on `(user_id, clip_id)`; order clips that this user has NOT used recently ahead of clips they have. Implemented as `ORDER BY (last_used_at IS NULL) DESC, last_used_at ASC NULLS FIRST, clip_id ASC` over the user's last-N usage window.

**Total ordering invariant:** the ORDER BY pipeline is a strict total order — there is never a tie. This is what makes the planner bounded-deterministic at every input combination.

**Output guarantees (per `[TDD-BROLL]-K`):**

- Hook is always non-null. If the SQL ever returned empty for a hook role, `BRollPlanner` raises `BRollCoverageError` and the request fails loud — the CI coverage check (`ci/broll_coverage_check.py`) is the deploy-time guarantee that this never happens in production.
- Maximum three clips per plan (hook + context + cta_bg). Often fewer — T2 and T5 ship hook only.
- Plan shape is structurally bounded by the JSONB object form; the `chk_b_roll_plan_shape` CHECK validates it at write time.

**Integration:** Worker-STRATEGIST persists `b_roll_plan` + `structure_template` + `active_season` to the `generations` row and INSERTs `broll_clip_usage` rows for every non-null clip — all inside one `db.transaction()` (per S13). Worker-COMPOSE reads these and branches per template per `[TDD-VIDEO]-B`.

**5 templates (canonical 15.000s each, full timelines in `[TDD-BROLL]-C`):**

| ID | Name | Trigger | Hook | Context | I2V | CTA |
|----|------|---------|------|---------|-----|-----|
| **T1** | Bookended (Default Sensory) | `(emotion, sensory category, mass)` and `DEFAULT_TEMPLATE` fallback | 3s clip | — | 9s | 3s clip |
| **T2** | Product-Dominant | `(logic, functional category, premium/luxury)` | 2s clip | — | 11s | 2s text-card |
| **T3** | Problem-Story | `(logic, functional category, mass)` | 3s clip | 4s clip | 5s | 3s text-card |
| **T4** | Festive Moment | `active_season != NULL` (override) | 3s festive clip | 4s festive clip | 5s | 3s text-card with festive accent |
| **T5** | Offer-Forward | `(conversion, *, *)` | 2s clip | — | 10s | 3s text-card with "LIMITED TIME" |

**`TEMPLATE_BY_CONTEXT`** is a frozen Python dict in `app/broll/templates.py`, CI-locked per Z9. Keys are `(framework_angle, category_class, brand_tier)` triples; values are template IDs. `CATEGORY_CLASS` maps `green_zone_category` enum values to `sensory` / `functional`. `MOOD_BY_ANGLE` is a frozen lookup used by the planner's mood filter inside each role+template+category triple.

**Tier-2 degradation interaction:** when `tier2_degradation_enabled = TRUE` (Founder-toggled via psql), the planner short-circuits to T1 with hook + text-card-CTA only, even if the routing dict would have selected T2..T5. This is the "library coverage gap" emergency path per `[TDD-BROLL]-I` and §13.9 — clarified in v2 (Conflict C9).
...
§9 is invoked by §8 (Phase 2 chain, Phase 4 coordinator). §11 hosts the `ad_framework` ENUM, the `broll_clips` table, and the v4-Δ1 `broll_clip_usage` and `seasons` tables. §12's `/chat`, `/regenerate`, `/advance` routes are the user-facing surfaces of this section. §15 lists the CI tests that lock this orchestration.
...
The v4-Δ1 B-roll work introduces no new screens, no new components, and no new design tokens. The HD-5 render progress component's existing "B-roll" stage row simply reads `strategy_card.b_roll_available` per §6.9 to decide whether to render. HD-4's Strategy Card display picks up `structure_template` and `active_season` as additional summary lines (UX freeze Z3 not violated — the layout is unchanged, only one or two more rows of text inside the existing summary block).
...
> **v2 note:** The `BRollClip` and `BRollPlan` TypeScript interfaces (from `[TDD-TYPES]-A` v4-Δ1) live in `shared/types/generation.ts` alongside `GenerationState`. The `StrategyCardV3.b_roll_plan` field is now typed `BRollPlan | null` (object) rather than `BRollClip[]` (array); `structure_template`, `active_season`, and `b_roll_available` are new fields on the strategy card. Storybook fixtures MOCK-06/07/08 (HD-4 strategy variants) need a one-time update to include these fields; this is a frontend-team task captured in §16.13 (Current Build Reality).
...
### 11.4 Postgres Tables (Catalog — v2 updates inline)

| Table | Purpose | Key Columns / Constraints | Notes |
|---|---|---|---|
| **`generations`** | The JSONB god-table; one row per generation | `gen_id PK`, `user_id FK`, `status job_status`, `pre_topup_status job_status NULL`, `plan_tier`, `routed_frameworks ad_framework[]`, `raw_scripts JSONB[3]`, `chat_turns_used 0..3`, `export_retry_count 0..3`, `strategy_card JSONB`, **`b_roll_plan JSONB` (object-shaped `{template, hook, context, cta}` per v4-Δ1; NULL permitted)**, **`structure_template VARCHAR(5) CHECK IN ('T1','T2','T3','T4','T5')`**, **`active_season VARCHAR(30) REFERENCES seasons(season_tag)`**, **`b_roll_performance_signal JSONB` (reserved for v2 attribution loop, unpopulated in v1)**, `preview_url`, `exports JSONB`, `cogs_total DECIMAL`, `declaration_*` provenance columns, `dlq_dead_at`, `dlq_original_task`. Constraints: `chk_pre_topup_coupling`, `chk_routed_frameworks_cardinality`, `chk_product_brief_shape`, `chk_exports_shape`, `chk_strategy_card_shape`, **`chk_b_roll_plan_shape` (rewritten in v4-Δ1: object form with `template ∈ {T1..T5}` enum guard; NULL permitted explicitly)**, **`chk_structure_template`**. | Pillar 4 (Deep Module). All Phase 1–4 fields in one row. |
| **`users`** | User identity + plan + session versioning | `user_id PK`, `google_id UNIQUE`, `email UNIQUE`, `plan_tier`, `credits_remaining`, `plan_expires_at`, `session_version` (instant token revocation), `beta_invited` | `idx_users_beta_invited` partial. Canonical PK target for all FKs is `users(user_id)`. |
...
| **`user_style_profiles`** | pgvector Style Memory (one row per user × category) | `user_id FK`, `category UNIQUE`, `language`, `motion_archetype`, `environment_preset`, `preferred_framework ad_framework`, `embedding vector(1536)`, `export_count`, **`brand_tier VARCHAR(10) CHECK IN ('mass','premium','luxury') DEFAULT 'mass'`** *(v4-Δ1)*, **`brand_color_primary VARCHAR(7) CHECK hex-format NULL`** *(v4-Δ1)* | Index: `ivfflat (embedding vector_cosine_ops) WITH (lists=100)`. Active gating: `export_count >= 2`. Cold-start defaults to `brand_tier='mass'`, `brand_color_primary=NULL` → neutral charcoal fallback in Worker-COMPOSE. |
...
| **`broll_clips`** *(v4-Δ1 — extended)* | Static B-Roll library, role-first | `clip_id VARCHAR(20) PK`, `archetype`, `category green_zone_category`, `duration_ms 1000..5000`, `r2_url`, `excludes_faces/hands/locations BOOLEAN NOT NULL` (all TRUE per `chk_broll_safety`), `is_active`, `license_ref`, **`role VARCHAR(15) CHECK IN ('hook','context','cta_bg')`**, **`mood_energy VARCHAR(10) CHECK IN ('calm','active','intense')`**, **`mood_temperature VARCHAR(10) CHECK IN ('warm','cool','neutral')`**, **`negative_space_score SMALLINT 0..10`**, **`seasonality_tags TEXT[] DEFAULT '{}'`** (empty = year-round), **`brand_tier_fit TEXT[] DEFAULT '{mass,premium,luxury}'`**. Additional constraint: **`chk_cta_negative_space`** — `role <> 'cta_bg' OR negative_space_score >= 7`. | Indices: `idx_broll_archetype_category` (partial), **`idx_broll_role_category_tier`** (partial btree on `(role, category, brand_tier_fit) WHERE is_active=TRUE` — planner hot path), **`idx_broll_seasonality`** (GIN on `seasonality_tags WHERE is_active=TRUE AND cardinality(seasonality_tags) > 0` — T4 selection branch). |
| **`broll_clip_usage`** *(NEW v4-Δ1)* | Per-(user, clip, gen) rotation history; planner deprioritizes clips this user used in their last N generations | `user_id UUID NOT NULL REFERENCES users(user_id) ON DELETE CASCADE`, `clip_id VARCHAR(20) NOT NULL REFERENCES broll_clips(clip_id) ON DELETE CASCADE`, `gen_id UUID NOT NULL REFERENCES generations(gen_id) ON DELETE CASCADE`, `used_at TIMESTAMPTZ DEFAULT NOW()`. PRIMARY KEY `(user_id, clip_id, gen_id)`. | Index: **`idx_broll_usage_user_recent`** btree on `(user_id, used_at DESC)`. CASCADE on all three FKs — history follows parent row retirement. **Retention:** no v1 prune job; at 3K users × ~1 gen/day × ≤3 clips/gen × 90 days ≈ 810K rows, trivial for Postgres. Re-evaluate at 10K MAU; flagged as F5 in TDD V5. |
| **`seasons`** *(NEW v4-Δ1)* | Lookup table for T4 festive-template override; ops/marketing toggle `is_active` to enable/disable without code deploy | `season_tag VARCHAR(30) PK`, `display_name VARCHAR(60)`, `active_from DATE`, `active_to DATE`, `is_active BOOLEAN DEFAULT FALSE`. Constraint: `chk_season_window` (`active_from <= active_to`). | Seeded with six Indian-market windows (`diwali`, `holi`, `wedding_season`, `monsoon`, `new_year`, `raksha_bandhan`), all `is_active=FALSE` at migration time. Founder/operator flips `is_active=TRUE` once the curated festive clip set lands — see §3.4 decision-rights matrix. |
| **`feature_flags`** | Toggles for Tier 1–4 degradation modes + beta gate + **`broll_v4_routing_enabled`** *(v4-Δ1, default FALSE)* | `key TEXT PK`, `value JSONB`, `description`, `updated_at` | Founder toggles via psql; agent never writes. |
...
`structure_template`, `role`, `mood_energy`, `mood_temperature`, and `brand_tier` are **CHECK-constrained VARCHARs**, not new Postgres ENUMs. This is deliberate: extending these vocabularies post-launch becomes a one-line `ALTER TABLE` CHECK update rather than an ENUM-rotation operation (which is much more invasive at scale). ENUMs remain reserved for the FSM-grade vocabularies that have trigger-enforced semantics.
...
├── broll/                            # curatorial folder structure — organizational only, not semantic
│   ├── ...                           # see note below
```

**B-roll folder taxonomy** *(v2 clarification per Conflict C8):* the `/broll/` subtree is organized at curation time for curator convenience; **the folder name does not constrain runtime selection**. The planner reads `broll_clips` row metadata (`role`, `category`, `mood_*`, `seasonality_tags`, `brand_tier_fit`) and dereferences `r2_url` — folder names are opaque to it. The PRD v3 enumeration "abstract / motion / texture / warehouse / packaging" is preserved as a curatorial convention only and is no longer authoritative; PRD v4 reframes it as such. The string "warehouse" is being retired from the taxonomy because it implies an identifiable location (contradicts U1 / `chk_broll_safety`). Re-naming the existing folders is **not** required for launch — the migration would be cosmetic and adds risk during the launch window. Re-evaluate post-launch.
...
**Retention:** unchanged from BEF v1 — Source/isolated/preview: 7d Starter, 30d Paid (DPDP §8(7)). Exports: 5 years (IT Rules 2026 §5). Audit logs (Postgres, partitioned): 5 years.

**Sweep cadence:** unchanged — daily 02:00 IST `retention_sweep`, weekly Mon 02:30 IST `r2_orphan_sweep`, daily 02:15 IST `partition_rotator`.
...
### 11.9 Worker Permission Matrix (Authority on Reads/Writes — v2 updated for v4-Δ1)
...
| **STRATEGIST** *(v2 — updated)* | safe_scripts, style_memory (incl. `brand_tier`, `brand_color_primary`), health_scores, cogs_estimate, director_tips, chat_turns_used, broll_clips, **seasons**, **broll_clip_usage** | strategy_card, **b_roll_plan (object-shaped)**, **structure_template**, **active_season**, **broll_clip_usage (INSERT)** | wallet, render, publish, **ALL external APIs (incl. embedding-library imports)**, **pre_topup_status** |
...
| **COMPOSE** *(v2 — updated)* | tts, selected_i2v, benefit, b_roll_plan, **structure_template**, broll_clips (DB fetch for clip download), **user_style_profiles (`brand_tier`, `brand_color_primary` — read via strategy_card payload)** | preview_url | wallet |
...
**The three zero-knowledge invariants** *(v2 — extended):*

1. **Worker-STRATEGIST has zero external API access.** CI: `ci/strategist_sandbox_check.py` AST-parses imports; blocks `httpx` / `requests` / `aiohttp` / `openai` / `anthropic` / `google.genai` / `boto3` in `app/workers/strategist.py`. **v4-Δ1:** the same check extends to all of `app/broll/` and additionally blocks `sentence_transformers`, `chromadb`, `faiss`, `faiss-cpu`, `transformers`, `torch` — defense against drift toward embedding-based clip selection (per `[TDD-BROLL]-J`).
...
3. **The B-roll routing dict is frozen.** `app/broll/templates.py` (TEMPLATE_BY_CONTEXT + MOOD_BY_ANGLE) is CI-locked per Z9. Snapshot test fails on any change.
...
The v4-Δ1 schema additions (5 new columns on `generations`, 2 new columns on `user_style_profiles`, 6 new columns on `broll_clips`, 2 new tables `broll_clip_usage` + `seasons`, 6 new constraints, 3 new indices) are all additive (`ADD COLUMN IF NOT EXISTS`, `CREATE TABLE IF NOT EXISTS`, `CREATE INDEX CONCURRENTLY IF NOT EXISTS`) and comply with the migration safety policy. The one CHECK constraint rewritten — `chk_b_roll_plan_shape` — is dropped-then-re-added with a backfill UPDATE between, all inside one transaction (per TDD V5 migration step in `[TDD-MIGRATIONS]`).

### 11.11 External Services & Credentials

This subsection defines all external services, infrastructure dependencies, and required environment variables for ADCREO MVP. All credentials must be stored in `.env.local` (development) and a secure secret manager (production). No secrets are committed to version control.
...
- `R2_BUCKET_NAME=adcreo-dev-assets`
- `R2_ENDPOINT_URL=`
- `R2_PUBLIC_URL=`

Purpose: store all media assets (preview, exports, B-roll). Used by workers via boto3 (NOT presigned URLs — see §11.8).

B-roll structure: `/broll/` — folder names are organizational-only, not semantic; runtime selection is metadata-driven via `broll_clips` row columns (see §11.8).
...
**IF** audit_log INSERT succeeds but Lua lock subsequently fails (Step 7 → `awaiting_funds`) → **THEN** the newly-inserted row is **kept** — legally valid regardless of money movement. On topup resumption, **IF** declaration is still fresh (<24h since the row inserted in Step 4) → **THEN** the next retry skips Step 4 re-sign; **ELSE** re-sign required.
...
> **v2 note:** The `state_change` event payload at `preview_ready` carries the standard fields; clients optionally hydrate `structure_template` via `GET /api/generations/{gen_id}` if they need to render template-specific affordances — but the HD-5/HD-6 UI is template-agnostic in v1, so this field is unused at launch.
...
### 13.9 Tier 1–4 Graceful Degradation Feature Flags *(v2 — Tier-2 rewritten)*

Per `[TDD-MIGRATIONS]` (step 20 + step 26 v4-Δ1) and `[TDD-BROLL]-I`, the following feature flags exist for emergency degradation. Founder toggles via psql; agent never writes.

| Flag | Effect When ON |
|---|---|
| `tier1_degradation_enabled` | pgvector heuristic fallback (Style Memory uses last-row-by-category instead of vector search) |
| `tier2_degradation_enabled` *(v2 — rewritten)* | **B-roll library coverage gap** — planner short-circuits to T1 with hook + text-card CTA only; Worker-COMPOSE skips context/cta-clip lookups and uses neutral text-card backdrop (`#1A1A1A`). Canonical 15s preserved. Replaces the v1 wording "B-Roll intercut disabled (9s I2V-only)" which violated the canonical 15s timeline. |
...
| **`broll_v4_routing_enabled`** *(NEW v4-Δ1, default FALSE)* | When FALSE: Worker-STRATEGIST short-circuits to T1 with `context=None, cta=None` — exact v4 baseline behavior preserved. When TRUE: full 5-template router active. This is the **rollout switch**, NOT a degradation flag — it stays OFF until the curated clip set lands and the coverage CI is green. |

Default: all degradation flags OFF; `broll_v4_routing_enabled` OFF (T1-only mode for safe rollout). Toggling a degradation flag is a P1/P2 incident response; toggling `broll_v4_routing_enabled` is a planned rollout action.
...
> **v2 note:** Every clip in `broll_clips` carries the hard CHECK `chk_broll_safety` (`excludes_faces = excludes_hands = excludes_locations = TRUE`). This is the legal anchor for U1 (Product-Visual First); the v4-Δ1 role-first taxonomy does not weaken this — every `role` value (hook, context, cta_bg) is subject to the same CHECK. The `chk_cta_negative_space` v4-Δ1 addition is an aesthetic / legibility check, not a compliance check; it does not write to `compliance_log`.
...
### 14.8 The Three Mandatory Declaration Checkboxes
...
### 14.9 Auto-Takedown Pipeline (Per `[TDD-TAKEDOWN]-A/B/C/D`)
...
| Grafana alert tier `GA-Critical` escalation | T+90 minutes if unresolved | `takedown_sla_watchdog` cron (every 5 min) |
...
- Queries `grievances WHERE status='open' AND created_at < NOW() - INTERVAL '22 hours'` → sends Grafana alert.
- Queries `grievances WHERE status='open' AND created_at < NOW() - INTERVAL '13 days'` → escalates to Founder.

### 14.10 Data Minimization (DPDP §8(7)) — Retention Sweep
...
### 14.11 Dependencies with other sections

§14 is the compliance implementation of §5's L1..L8 invariants. §11 provides the `audit_log`, `compliance_log`, and `grievances` tables. §12's `/declaration` and `/retry-export` endpoints insert into `audit_log` per §14.7. §13's DLQ handler triggers refund before takedown in parallel.
...
- The five-layer test pyramid (unit / integration / contract / E2E / manual).
- The CI job inventory with red/green criteria, **v2 — extended with `ci/broll_coverage_check.py` and `ci/strategist_sandbox_check.py` scope extension**.
- The mapping between PRD acceptance criteria and CI jobs.
- The Founder's manual verification checkpoints corresponding to the five approval gates.
- The Beta gate criteria and MVP gate criteria.
- The rollback-drill specification (corrected in v2 to use raw-SQL migration runner per C6, not Alembic).
- The observability and SLA monitoring surface (v2 normalizes alert labels to `GA-Warn` / `GA-Critical` per C7).
...
### 15.5 The CI Job Inventory `[TDD-CICD]-A..E` *(v2 — extended)*
...
| **F** *(NEW v4-Δ1)* | `ci/broll_coverage_check.py` | Library coverage for every routable `(angle, category_class, brand_tier)` triple AND every active season | (i) For every key in `TEMPLATE_BY_CONTEXT`, assert ≥ 1 active clip exists for each role of the resolved template. (ii) For every `seasons.is_active = TRUE` row, assert ≥ 1 hook + 1 context clip has the matching tag in `seasonality_tags`. (iii) Fails the build on any uncovered combination — deploy is blocked. |
| **G** *(extended v4-Δ1)* | `ci/strategist_sandbox_check.py` | Import graph for the strategist + B-roll modules | Existing: zero `httpx` / `requests` / `aiohttp` / `openai` / `anthropic` / `google.genai` / `boto3` in `app/workers/strategist.py`. **v4-Δ1:** the same rule extends to all of `app/broll/` (prefix-match `app/broll/`) and additionally blocks `sentence_transformers`, `chromadb`, `faiss`, `faiss-cpu`, `transformers`, `torch`. |
| **H** *(NEW v4-Δ1, optional but recommended)* | `ci/cross_doc_b_roll_consistency.py` | PRD/BEF/TDD do not drift on B-roll terminology | Greps all three docs for the deprecated taxonomy strings (`abstract.*motion.*texture.*warehouse.*packaging` as a single sentence) and the old planner signature (`BRollPlanner\.plan\(framework_angle,\s*category\)`); fails if any match is found outside an explicit "historical / superseded" block. Five-line script; catches future drift cheaply. |

**Build seam:** all jobs run as required GitHub Actions checks on every PR. `main` has branch protection requiring all green plus L1+L2+L3 layers green.

**Observability Setup:**
- Skeleton credentials provisioned for PostHog and Grafana Cloud.
- No live telemetry required at this stage.
- PagerDuty excluded; alerts handled via Grafana (`GA-Warn` / `GA-Critical` tiers per Conflict C7 normalization).
...
**v2 note:** the AC-4 (HD-4 Strategist) suite is extended to cover the v4-Δ1 strategist contract — verifies `b_roll_plan` is object-shaped, `structure_template` is one of T1..T5, `broll_clip_usage` rows are written atomically with the plan UPDATE, and the feature flag `broll_v4_routing_enabled` correctly gates the router.

### 15.7 Founder's Manual Verification Checkpoints (at the §3 Gates) *(v2 — corrected per C5 / C6)*
...
|---|---|---|
| Gate 1 — Foundation Sign-Off | After §11 schema, §7 FSM, §10 design tokens are wired; verified by successful ECM-007 middleware response | (i) `ci/run_migrations.py --apply` clean on a fresh DB *(corrected v2 — no `alembic` per C6)*; (ii) seeded ENUMs match `[TDD-ENUMS]`; (iii) audit_log REVOKE policy verified by attempting an UPDATE as the app role and confirming permission denied; (iv) CI-A green. |
...
| Gate 3 — Phase 2 + Phase 3 Sign-Off *(v2 — extended for v4-Δ1)* | After HD-3 (Co-Pilot) + HD-4 (Strategist + B-roll v4-Δ1 router) operational | (i) Walk a bounded chat; confirm chat depth cap enforced; (ii) confirm Strategist returns a credit-locked plan with `b_roll_plan` object-shaped, `structure_template` set, `active_season` either set or NULL, and 1–3 rows in `broll_clip_usage` for the gen; (iii) confirm credit lock occurs **before** Phase 4 enqueue; (iv) confirm `ci/broll_coverage_check.py` and `ci/strategist_sandbox_check.py` are green; (v) AC-3 + AC-4 green. |
| Gate 4 — Phase 4 Sign-Off (Compliance Heavy) *(v2 — corrected per C5)* | After Worker-COMPOSE (template-branched), Worker-EXPORT, C2PA, SGI operational | (i) Visually inspect SGI burn-in on both 1080×1080 and 1080×1920 exports; (ii) **C2PA manifest validates using local `c2patool --verify`. External verification APIs (Adobe VerifyContentCredentials) are strictly forbidden in both CI and runtime per `[PRD-FEATURES-COMPLIANCE]`** *(v2 — replaces v1 wording that required Adobe; reconciled to C5 + GAP-07)*; (iii) confirm `compliance_log` row written with `check_type='c2pa_sign'`; (iv) flip `broll_v4_routing_enabled = TRUE`; render one gen each at T1 and T3 (or T2 if `(logic, functional, premium)` is curated); verify Worker-COMPOSE outputs canonical 15s with correct segment boundaries per `[TDD-BROLL]-C`; (v) verify the FFmpeg-rendered text-card CTA appears with the correct brand color (or `#1A1A1A` fallback) for T2..T5; (vi) AC-5 + AC-6 green. |
| Gate 5 — Pre-Beta Sign-Off | After E2E green + rollback drill executed | (i) Run the rollback drill in §15.10; (ii) confirm SLA Watchdogs paging via Grafana; (iii) confirm grievance pipeline completes a synthetic takedown end-to-end; (iv) all CI jobs green (incl. F + G + H); (v) all `[PRD-AC-1..6]` E2E suites green. |
...
| E1..E6 | Engineering | CI + L4 E2E | All CI jobs green; L4 suite green; observability dashboard live per `[PRD-FEATURES-INFRA]` F-606 |
...
### 15.10 The Rollback Drill — `[TDD-ROLLBACK]-F` *(v2 — corrected per C6)*
...
4. Trigger rollback via the documented runbook: roll back the application Docker image per `[TDD-ROLLBACK]-G`; for schema rollback, execute the matching `rollback_*.sql` file via `ci/run_migrations.py --rollback` *(v2 — corrected; v1 incorrectly referenced `alembic downgrade -1` and `Helm rollback`; ADCREO uses raw SQL migrations and a single Docker image, no Helm)*.
5. Confirm the in-flight generation lands cleanly in `failed_render` or `failed_export` with refund issued (DLQ Branch behaviour preserved across rollback).
...
8. Re-run all CI jobs against the rolled-back deploy; all must remain green.
9. Founder logs the drill in `docs/rollback_drills/<YYYY-MM-DD>.md`.

### 15.11 Observability and SLA Monitoring *(v2 — alert labels normalized per C7)*

Per `[PRD-FEATURES-INFRA]` F-601..F-607 and `[TDD-OBSERVABILITY]`:

| Signal | Source | Threshold | Page |
|---|---|---|---|
| Generation success rate (rolling 1h) | `generations` table | < 92% | GA-Warn |
| Phase 4 p95 latency | ARQ job runtime | > 240s | GA-Warn |
| C2PA sign failure rate | `compliance_log` | > 1% | GA-Critical |
| Grievance ACK SLA breach | `takedown_sla_watchdog` | any breach | GA-Critical |
| Idempotency cache hit-rate | Redis DB5 | < 5% on retry traffic | GA-Warn (suggests cache disabled) |
| Credit lock hold time p95 | Redis DB2 | > 6s | GA-Warn |
| DLQ depth | ARQ DLQ list | > 25 jobs | GA-Critical |
| **B-roll planner SQL latency p95** *(NEW v4-Δ1, optional)* | application metric | > 50ms | GA-Warn (suggests rotation join is regressing) |
...
- **(NEW v2)** The current build reality snapshot (§16.13) — where the build is *right now* relative to the original sprint plan.
...
The 15-day build window is partitioned into seven micro-phases. This is preserved verbatim from BEF v1 as historical record; current reality is in §16.13.

| # | Window | Micro-phase | Owning BEF Sections | Exit Condition |
|---|---|---|---|---|
| 1 | Days 1–3 | **Foundation** — repo scaffold, design tokens, FSM seed, schema migration v1, ENUM seed, audit_log REVOKE policy | §7, §10, §11 | Gate 1 Verified |
| 2 | Days 4–5 | **Phase 1 Build** — HD-1 upload, Worker-EXTRACT, HD-2 Director Review, confidence emit, intent gate | §6, §8 (Phase 1), §9 (Director chain), §10, §12 (`/generations`) | Gate 2 cleared |
| 3 | Days 6–7 | **Phase 2 + Phase 3 Build** — HD-3 bounded Co-Pilot, Worker-PHASE2, script lock, HD-4 Strategist, asset selection (v4-Δ1 router), credit lock | §6, §8 (Phases 2+3), §9 (Co-Pilot + Strategist + v4-Δ1 planner), §10, §12 (`/copilot`, `/strategy`) | Gate 3 cleared |
| 4 | Days 8–10 | **Phase 4 Build (Heavy)** — Worker-COMPOSE (template-branched), FFmpeg filter graph (variable-arity), Worker-EXPORT, C2PA signing, SGI burn-in, HD-5 progress, HD-6 declaration | §6, §8 (Phase 4), §10, §12 (`/render`, `/declaration`, `/export`), §14 | Gate 4 cleared |
| 5 | Days 11–12 | **Cross-Cutting Hardening** — DLQ branches, refund / retry-export, idempotency middleware, payment FSM, webhook origin-restore, retention sweeps, takedown pipeline | §11 (concurrency), §12 (`/retry-export`, `/webhook`), §13, §14.9–§14.10 | All CI jobs green |
| 6 | Days 13–14 | **L4 E2E + Observability** — full Playwright walks for AC-1..AC-6, observability dashboard wiring, SLA watchdogs, on-call rota | §15.5–§15.6, §15.11 | E2E suite green; dashboards live |
| 7 | Day 15 | **Pre-Beta Lock** — rollback drill, MVP gate dry-run, freeze of `main` | §15.10, §16.5 | Gate 5 cleared → enter closed beta |
...
| Gate 3 | 3 (Phase 2+3) | §15.7 row 3 + AC-3 + AC-4 green + `ci/broll_coverage_check.py` green + `ci/strategist_sandbox_check.py` green | Founder posts `unblock: gate-3` |
| Gate 4 | 4 (Phase 4) | §15.7 row 4 + C2PA manifest validates using local `c2patool --verify` (external verification APIs forbidden per `[PRD-FEATURES-COMPLIANCE]`) + AC-5 + AC-6 green | Founder posts `unblock: gate-4` |
| Gate 5 | 6 (E2E + Observability) | §15.7 row 5 + rollback drill log entry + all CI jobs green (incl. F + G + H) + all dashboards live | Founder posts `unblock: gate-5 → enter-beta` |
...
### 16.7 Open Gaps and Reconciliation Tickets *(v2 — extended)*
...
| GAP-03 | Idempotency TTL on 4xx responses | PRD `[PRD-IDEMPOTENCY]` says cache 2xx only; TDD `[TDD-CONCURRENCY]-A` confirms. CI-C enforces. No gap; flagged for visibility. | 5 | None — already enforced |
...
| GAP-06 | Grievance form UI surface | §14.9 specifies the pipeline; PRD `[PRD-FEATURES-COMPLIANCE]` F-505 references the form. Lives at `/grievance` outside the 6-screen flow per §10. | Post-MVP | Resolved — outside linear flow |
| GAP-07 | C2PA verification target | §15.7 Gate 4 v1 wording required Adobe verification (external dependency). **v2 resolution:** local `c2patool --verify` is the canonical path; external APIs forbidden in CI and runtime per `[PRD-FEATURES-COMPLIANCE]`. Conflict C5. | 4 | **Resolved in v2** |
| GAP-08 | Rollback drill cadence post-launch | §15.10 specifies pre-launch and at-deploy. PRD `[PRD-RELEASE-MVP]` M9 says "past 7 days." The post-MVP cadence is not specified. | Post-MVP | Founder sets ongoing drill cadence post-launch |
| GAP-09 | Migration runner | Conflict C6. **Resolved:** raw SQL via `ci/run_migrations.py`, no Alembic. v1 §15.10 references to `alembic` corrected in v2. | — | **Resolved in v2** |
| GAP-10 | `confidence_score` computation source | Decision: compute deterministically from BiRefNet alpha mask (`foreground_ratio × edge_sharpness_penalty`). Output contract float ∈ [0.0, 1.0] unchanged. | 2 | **Resolved — Founder + Architect approved** |
| GAP-11 | HD-3 script tile count on partial SAFETY pass | Render only safe tiles (1, 2, or 3); SSE payload gains `scripts_available` and `rejected_frameworks`. Zero-tile state impossible at `scripts_ready`. UI pre-selects tile 1 by CRITIC score regardless of count. | 3 | **Resolved 2026-05-07** |
| GAP-12 | Worker-STRATEGIST cold-start style defaults *(v2 — corrected)* | v1 GAP-12 referenced `fashion` and `home_decor` as keys in `COLD_START_DEFAULTS`, but **these are not Green Zone categories** (§4.7 only allows `d2c_beauty`, `packaged_food`, `hard_accessories`, `electronics`, `home_kitchen`). **v2 corrected defaults**, keyed only by Green Zone values: `packaged_food` → `{motion: "gentle_zoom", environment: "kitchen_warm"}`; `d2c_beauty` → `{motion: "product_reveal", environment: "clean_white"}`; `electronics` → `{motion: "spec_overlay", environment: "minimal_studio"}`; `hard_accessories` → `{motion: "rotation_showcase", environment: "neutral_studio"}`; `home_kitchen` → `{motion: "ambient_float", environment: "living_room_warm"}`. Fallback (defensive only — should never trigger because Worker-EXTRACT rejects non-Green-Zone before STRATEGIST runs): `{motion: "gentle_zoom", environment: "clean_white"}`. | 3 | **Resolved in v2 — Architect rewrote to align with Green Zone enum** |
| **GAP-13** *(NEW v2)* | B-roll clip library curation backlog | The v4-Δ1 architecture is in place but the curated 25–30 clip library is only partially populated. `ci/broll_coverage_check.py` will block deploy until every routable `(angle, category_class, brand_tier)` triple has ≥ 1 active clip per required role. Until then, `broll_v4_routing_enabled` must stay FALSE — the strategist short-circuits to T1, exact v4 baseline behavior. | 4 (Phase 4 build) and 7 (Pre-Beta) | Founder + Curator complete library before Gate 4 sign-off; until then ship with flag OFF |
| **GAP-14** *(NEW v2)* | Seasons table activation cadence | `seasons` is seeded with `is_active=FALSE` on all six rows. Ops/marketing must flip the relevant row to TRUE before Diwali / Holi / wedding-season / etc. windows. No code change required; psql `UPDATE seasons SET is_active=TRUE WHERE season_tag=$1`. T4 (Festive Moment) template is dormant until first activation. | Post-launch (per-season) | Founder + Ops calendar — first activation likely raksha-bandhan (Aug) or diwali (Oct) |
| **GAP-15** *(NEW v2)* | PRD V3 → V4 sync | PRD V3 still describes B-roll using the deprecated taxonomy ("abstract, motion, texture, warehouse, packaging") at lines 234, 408, 2099. TDD V5 and BEF V2 use the role-first taxonomy. PRD V4 ticket open — three surgical edits (≤30 min). Until PRD V4 is merged, **TDD/BEF take precedence on Axis A2** (implementation taxonomy) per §2.4. | Post-launch / day-2 | Founder publishes PRD V4 within first week post-launch |
| **GAP-16** *(NEW v2)* | BEF v1 references to "Adobe VerifyContentCredentials" | v1 §15.7 Gate 4 + v1 GAP-07 required external Adobe verification, contradicting `[PRD-FEATURES-COMPLIANCE]` and TDD `[TDD-WORKERS]-I`. **Resolved in this v2 revision** — all references replaced with local `c2patool --verify`. | — | **Resolved in v2** |

### 16.8 Frozen Out-of-Scope Items (Post-MVP)

Per `[PRD-UX-FREEZE]`, the following are **explicitly out of scope** for v1.0 and must not be authored, scaffolded, or stubbed by the Agentic IDE:

- Multi-language UI (English at launch; Hindi/Hinglish/Marathi/Punjabi/Bengali/Tamil/Telugu via TTS only, not UI).
...
- Self-serve refund without DLQ trigger.
...
- **User-uploaded custom B-roll** *(v4-Δ1 — explicitly deferred per `[TDD-BROLL]-I`; curation discipline > user variance for v1).*
- **B-roll performance feedback loop** *(v4-Δ1 — schema reserved as `b_roll_performance_signal JSONB`, unpopulated; v2 work pending 4–6 weeks of run data).*
- **pgvector / embedding-based B-roll selection** *(v4-Δ1 — explicitly rejected per `[TDD-BROLL]-I`; CI-locked via `ci/strategist_sandbox_check.py` extension to `app/broll/`).*
- **Brand-palette filtering of clips beyond `brand_tier_fit`** *(v4-Δ1 — `brand_tier_fit` array provides 80% of the value at 10% of the complexity; deferred).*

### 16.9 Sprint Calendar (Historical — Preserved)

The original sprint calendar from BEF v1 is preserved verbatim as historical record. Current reality is in §16.13.
...
| May 6 | Cross-cutting hardening complete; all CI jobs green |
...
The BEF v2 is considered done when all of the following hold:
...
3. Every TDD anchor referenced in §2.5 maps to at least one BEF section that sequences it (TDD V5 anchors including `[TDD-BROLL]-A..K`).
...
7. No section invents a feature, screen, state, ENUM value, or endpoint not present in PRD v3 (pending v4 sync) or TDD v5.
8. **(NEW v2)** Every B-roll surface (planner signature, schema row, permission line, strategy-card field, chip-change NULL list, Tier-2 description, R2 folder note) is consistent with TDD V5's `[TDD-BROLL]` chapter and the corresponding schema changes.

### 16.11 Closing Authority Statement

This Blueprint Execution File v2 is the operating contract under which Google Antigravity (the Agentic IDE) authors Adcreo code and the Founder reviews it. It is a **third-tier authority document**: subordinate to PRD on contract questions, subordinate to TDD V5 on implementation questions, and authoritative on execution sequencing, halt points, and gap resolution.

Any divergence between this BEF and PRD / TDD V5 on a non-execution matter is automatically resolved in favour of PRD or TDD per §2.4. Any divergence on an execution matter (order, gates, single-file scope, halt rules) is resolved in favour of this BEF.
...
### 16.13 Current Build Reality *(NEW v2 — as of 19 May 2026)*

This subsection captures the state of the build at the moment of BEF v2 authorship, independent of the original Apr 26 → May 10 sprint calendar in §16.9. It exists so a fresh agent session can orient itself without replaying historical commits.

**Position:** Micro-phase 5 (Cross-Cutting Hardening) is in progress on the VPS. The 1-week launch window is the immediate target.

**Gates cleared:**

- **G1 (Foundation):** ✅ cleared — schema migrations applied (including the v4-Δ1 additive migrations per TDD V5 §[TDD-MIGRATIONS] steps 22–26), FSM trigger in place, audit_log REVOKE policy verified.
- **G2 (Phase 1):** ✅ cleared — Worker-EXTRACT live, HD-1/HD-2 operational, confidence emit working, AC-1/AC-2 green.

**Gates partial:**

- **G3 (Phase 2 + Phase 3):** 🟡 partial. Phase 2 chain (Worker-COPY router + generate + CRITIC + SAFETY) is functional and AC-3 green. **Phase 3 / Worker-STRATEGIST v4-Δ1 router is partially built** — the legacy `BRollPlanner.plan(framework_angle, category)` 2-arg version is in place; the 6-arg v4-Δ1 contract (template + brand_tier + user_id + active_season + rotation history) needs to be wired before G3 closes. The `broll_v4_routing_enabled` flag is FALSE, so the legacy behavior is acceptable in staging; the new contract must land before the flag flips. AC-4 not yet fully green.
- **G4 (Phase 4):** 🟡 partial. Worker-COMPOSE + Worker-EXPORT + C2PA signing + SGI burn-in are functional in some form. **The v4-Δ1 template-branched FFmpeg graph is the open work** — Worker-COMPOSE needs the `structure_template` argument, the `_build_segments_for_template` method, and the `_render_cta_text_card` method per TDD V5 `[TDD-VIDEO]-B`. The variable-arity `filter_complex` concat is also new. AC-5 / AC-6 not yet fully green.

**Gates not yet attempted:**

- **G5 (Pre-Beta):** rollback drill, full L4 E2E, observability dashboards live.

**Immediate priorities for micro-phase 5 hardening (recommended order):**

1. Land the v4-Δ1 schema migrations on production-mirror staging (the additive migrations are already authored per TDD V5; just need `ci/run_migrations.py --apply` against staging).
2. Update `app/workers/strategist.py` to the 6-arg `BRollPlanner.plan(template, angle, category, brand_tier, user_id, active_season)` contract with atomic 5-step persistence (per §8.6 above).
3. Update `app/workers/compose.py` to the template-branched signature (per §8.7 step 6 + TDD V5 `[TDD-VIDEO]-B`).
4. Update `app/broll/planner.py` to add the rotation-deprioritization JOIN and the role-based filter pipeline (per §9.6 above).
5. Author `app/broll/templates.py` with the frozen `TEMPLATE_BY_CONTEXT` dict + `MOOD_BY_ANGLE` + `CATEGORY_CLASS` (the agent reads this file; only Founder may edit it per §3.4).
6. Author `ci/broll_coverage_check.py` (CI job F).
7. Extend `ci/strategist_sandbox_check.py` to cover `app/broll/` and the ML library blocklist (CI job G v4-Δ1 update).
8. Update frontend `StrategyCard` component to surface `structure_template` and `active_season` (one or two additional rows in the existing summary block; no layout change).
9. Update Storybook fixtures MOCK-06/07/08 to include the new fields.
10. Re-run G3 verification; flip `broll_v4_routing_enabled = TRUE` once the curated clip library passes the coverage check.

**Open library-curation work:** the curated 25–30 clip library is the long pole. Per GAP-13, `broll_v4_routing_enabled` cannot flip TRUE until coverage is complete. Until then, every render uses T1 — which is the exact v4 baseline behavior and is what the existing partially-built compose code already supports.

**Risk surface:** the launch is reachable in 1 week if (a) the strategist v4-Δ1 wire-up and the compose template branch land in 2–3 days of focused work, (b) the curated library is completed in parallel, and (c) G3 / G4 sign-off happen on time. The fallback — ship with `broll_v4_routing_enabled = FALSE` (T1-only mode) and flip the flag post-launch once the library is ready — is a perfectly acceptable launch posture; T1 is the v4 baseline and ships everything users need.

---

## V2 CHANGELOG

This BEF v2 supersedes BEF v1 in every surface touched by the v4-Δ1 B-roll architecture and corrects pre-existing inconsistencies surfaced during architect review. The changes below are enumerated in section order; sections not listed are unchanged from BEF v1.

**Document-level changes:**

1. **Header** — version v1 → v2; supersedes-line updated; status revised to "Aligned to PRD v4 (queued) + TDD v5 (19 May 2026)"; v4-Δ1 added to invariant list; **product name reconciled from "AdvertWise" to "Adcreo"** throughout (24+ instances).

**Per-section changes:**

2. **§2.5 (Anchor-to-Section Map)** — added F-405 B-Roll Planner row pointing to `[TDD-BROLL]` (full chapter) + new schema tables + Worker-STRATEGIST + `[TDD-VIDEO]-A/B`. Extended `[PRD-MOATS]` row to mention `brand_tier`, `brand_color_primary`.
3. **§2.7 (Identified Conflicts)** — added Conflict **C8** (B-Roll taxonomy reframed to role-first); added Conflict **C9** (Tier-2 degradation rewritten to T1-with-text-card path); annotated **C5** (Adobe verification removed from Gate 4); annotated **C6** (Alembic references corrected in §15.10); annotated **C7** (PD-Warn → GA-Warn label normalization).
4. **§3.4 (Decision-Rights Matrix)** — added rows: enabling a `seasons.is_active=TRUE` row (Founder-exclusive); editing `app/broll/templates.py` (Founder-exclusive). PagerDuty acknowledgment → Grafana acknowledgment per C7.
5. **§3.5 (Approval Gates G1-G5)** — **G3 rewritten** with the new bounded-determinism contract (5 verification points covering coverage check, fresh-user determinism, rotation-aware test, plan shape, and template lookup); **G4 rewritten** with local `c2patool --verify` (Adobe verification removed per C5) and template-branched compose verification.
6. **§3.6 (Forbidden Actions)** — extended sandbox to `app/broll/`; added ML library blocklist (sentence_transformers/chromadb/faiss/transformers/torch); added forbidden edits to `app/broll/templates.py` and `seasons.is_active`; clarified R2 folder names are curatorial-only.
7. **§4.5 (User Deliverable)** — Style Memory upsert now mentions `brand_tier` and `brand_color_primary`.
8. **§4.7 (Input Surface)** — "4 Red Zone" wording replaced with "All others" (since Red Zone is not a fixed enum, just the complement of Green Zone).
9. **§4.8 (Output Surface)** — Render duration row reframed: T1 (default Bookended) shown as canonical example; T2..T5 explicitly mention text-card CTA substitution.
10. **§4.9 (Competitive Positioning)** — Hyper-localization moat extended with festive template (T4) override.
11. **§4.10 (Plan Tier Contract)** — added new paragraph documenting `plan_tier × structure_template` decoupling (resolves TDD V5 F6) with `broll_v4_routing_enabled` global kill-switch.
12. **§5.4 (Financial Invariants)** — added **F9** (Zero Marginal Cost B-Roll Selection).
13. **§5.5 (Legal Invariants)** — added **L8** (B-Roll Safety Purity, covering `chk_broll_safety` + `chk_cta_negative_space`).
14. **§5.6 (UX Invariants)** — added **U6** (Role-First B-Roll); annotated U1 to note "warehouse" deprecation.
15. **§5.7 (System Predictability Invariants)** — added **S13** (B-Roll Plan Atomic Persistence) and **S14** (B-Roll Plan Shape — object, not array).
16. **§5.8 (Freeze Rules)** — added **Z9** (B-Roll Templates Freeze — CI-locked `templates.py`).
17. **§6.8 (HD-4 [Edit] back-navigation matrix)** — extended NULLed-downstream column for all 4 [Edit] targets to include `structure_template`, `active_season`, `b_roll_performance_signal`; added v2 explanatory note about `broll_clip_usage` not being deleted (rotation history persists).
18. **§6.9 (HD-5)** — rewritten "System does" paragraph to describe the template-branched FFmpeg pipeline; added the `b_roll_available` flag's role in showing/hiding the B-roll stage row.
19. **§8.5 (Phase 2 chip-change re-runs)** — extended NULL list to include `b_roll_plan`, `strategy_card`, `structure_template`, `active_season`, `b_roll_performance_signal`; clarified `broll_clip_usage` persistence.
20. **§8.6 (Phase 3)** — entire section rewritten: 5-step v4-Δ1 flow (active_season → brand_tier → structure_template → planner → atomic persist); 6-arg planner contract; extended reads/writes lists; updated CI scope.
21. **§8.7 (Phase 4 — Production)** — Worker-COMPOSE step (6) rewritten to describe template-branched FFmpeg graph, text-card CTA via lavfi color + drawtext, variable-arity concat, brand-color fallback.
22. **§9.6 (B-Roll Planner)** — entire section rewritten as the v4-Δ1 deterministic role-first planner: 6-arg signature, 8-step filtering pipeline (incl. role / brand-tier / seasonality / negative-space / rotation), total-ordering invariant, output guarantees, 5-template lookup table, Tier-2 degradation interaction. **G3 acceptance test from BEF v1 superseded** — byte-identical determinism on `(framework_angle, category)` is replaced by bounded determinism with rotation history as part of input.
23. **§11.4 (Postgres Tables)** — `generations` row extended with v4-Δ1 columns + new CHECK; `user_style_profiles` row extended with `brand_tier` and `brand_color_primary`; `broll_clips` row extended with 6 new columns + 2 new CHECKs + 2 new indices; **two new table rows added** — `broll_clip_usage`, `seasons`; `feature_flags` row updated to mention `broll_v4_routing_enabled`.
24. **§11.5 (ENUMs)** — added clarification that `structure_template`, `role`, `mood_energy`, `mood_temperature`, `brand_tier` are CHECK-constrained VARCHARs, not new Postgres ENUMs (deliberate, for migration flexibility).
25. **§11.8 (R2 bucket structure)** — bucket renamed `advertwise-assets` → `adcreo-assets`; added v2 clarification that `/broll/` folder names are organizational-only, not semantic; documented PRD-v3 taxonomy ("warehouse" etc.) as a deprecated curatorial convention pending PRD v4.
26. **§11.9 (Worker Permission Matrix)** — **STRATEGIST row** updated: added reads for `seasons`, `broll_clip_usage`, `brand_tier`, `brand_color_primary`; added writes for `structure_template`, `active_season`, `broll_clip_usage` (INSERT); denied list now mentions embedding-library imports. **COMPOSE row** updated: added `structure_template`, `broll_clips` DB-fetch, `user_style_profiles` (brand_tier/brand_color_primary) via strategy_card. **Three zero-knowledge invariants** list grew from two to three.
27. **§11.10 (Migration Safety Policy)** — added paragraph documenting that all v4-Δ1 additions are additive-only and that the one CHECK constraint rewrite (`chk_b_roll_plan_shape`) follows the drop-backfill-readd pattern inside a single transaction.
28. **§11.11 (External Services)** — §11.11.10 normalized to "Grafana Cloud — alerts handled via Grafana (no PagerDuty in MVP)" per C7.
29. **§13.9 (Tier 1–4 Degradation Flags)** — **Tier-2 row rewritten** (Conflict C9): "B-roll library coverage gap — planner short-circuits to T1 with hook + text-card CTA only" (replaces "9s I2V-only" which violated the canonical 15s timeline). New row added: `broll_v4_routing_enabled` (rollout switch, not degradation).
30. **§14 (Compliance)** — annotated "PagerDuty P0 escalation" → "Grafana alert tier `GA-Critical` escalation" per C7; added v2 clarification on B-roll compliance (`chk_broll_safety` extends to all role values; `chk_cta_negative_space` is aesthetic, not compliance).
31. **§15.5 (CI Job Inventory)** — added **CI job F** (`ci/broll_coverage_check.py`); **extended CI job G** (`ci/strategist_sandbox_check.py` scope + ML library blocklist); added **CI job H** (`ci/cross_doc_b_roll_consistency.py` — optional cross-document drift detector).
32. **§15.7 (Founder Manual Checkpoints)** — Gate 1 row corrected (`alembic upgrade head` → `ci/run_migrations.py --apply` per C6); Gate 3 extended with v4-Δ1 verification points (plan shape, usage rows, coverage CI, sandbox CI); Gate 4 corrected (Adobe → local `c2patool --verify` per C5) and extended (template-branched compose + text-card CTA verification); Gate 5 mentions new CI jobs F/G/H.
33. **§15.10 (Rollback Drill)** — step 4 corrected: `alembic downgrade -1` and `Helm rollback` replaced with raw-SQL rollback via `ci/run_migrations.py --rollback` + Docker image rollback per `[TDD-ROLLBACK]-G` (Conflict C6).
34. **§15.11 (Observability)** — all "PD-Warn / PD-Critical" labels normalized to "GA-Warn / GA-Critical" per C7; added optional v4-Δ1 row "B-roll planner SQL latency p95".
35. **§16.7 (Open Gaps)** — **GAP-07 resolved in v2** (Adobe → local c2patool); **GAP-09 resolved in v2** (Alembic → raw SQL); **GAP-12 corrected in v2** (cold-start defaults rekeyed to Green Zone categories — original v1 entry referenced `fashion` and `home_decor` which are NOT Green Zone categories, a pre-existing inconsistency); **GAP-13 added** (B-roll clip library curation backlog); **GAP-14 added** (seasons activation cadence); **GAP-15 added** (PRD V3→V4 sync); **GAP-16 added** (BEF v1 Adobe references resolved here).
36. **§16.8 (Frozen Out-of-Scope)** — added four v4-Δ1 explicit non-goals: user-uploaded custom B-roll, B-roll performance feedback loop (v2 work), embedding-based selection, brand-palette filtering beyond `brand_tier_fit`.
37. **§16.13 (NEW Current Build Reality)** — added entirely new subsection documenting the 19 May 2026 build state: G1/G2 cleared, G3/G4 partial; immediate priorities for micro-phase 5 hardening enumerated; library curation called out as the long pole; explicit fallback posture documented (ship with `broll_v4_routing_enabled=FALSE` — T1-only mode is the v4 baseline and is acceptable).

**Cleanups beyond v4-Δ1 (caught during architect review):**

- §15.7 Gate 1 + §15.10 step 4: replaced `alembic` references with `ci/run_migrations.py` per C6 (pre-existing v1 inconsistency).
- §15.7 Gate 4 + v1 GAP-07: replaced "Adobe VerifyContentCredentials" with local `c2patool --verify` per C5 (pre-existing v1 inconsistency).
- §15.11 + §3.4: normalized "PD-Warn / PD-Critical / PagerDuty P0" labels to "GA-Warn / GA-Critical / Grafana alert" per C7 (pre-existing v1 inconsistency).
- §16.7 GAP-12: rekeyed cold-start defaults to Green Zone categories only (pre-existing v1 inconsistency — `fashion` and `home_decor` are not Green Zone).
- Document-wide: instances of "AdvertWise" replaced with "Adcreo" / "ADCREO" throughout.

**Verification grep run during v2 authorship (zero hits expected, all hits accounted for in changelog):**

```
grep -nE "BRollPlanner\.plan\(framework_angle,\s*category\)" BEF_V2   # zero hits — old 2-arg signature gone
grep -nE "b_roll_plan JSONB ≤3"                                       # zero hits — array bound replaced
grep -nE "abstract.*motion.*texture.*warehouse.*packaging"            # zero hits in normative text (only in changelog + R2 historical note)
grep -ni "advertwise"                                                 # zero hits — rename complete
grep -ni "alembic\|helm rollback\|PagerDuty\|Adobe Verify"            # zero hits in normative text
```
